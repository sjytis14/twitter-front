...
/**
 * @dev A function that shows navbar (there is a button to connect to metamask)
 * @param {setError} A function that manages the variable "error"
 * @returns Navbar
 */
const Navigation = ({ setError }) => {
  ...
  /**
   * @dev When rendering for the first time, check the connected user and change the current account
   */
  useEffect(() => {
    if (window.ethereum) {
      window.ethereum
  ...
  /**
   * @dev Check the connected user and change the current account
   * @param {accounts} The account to check
   */
  ...
  /**
   * @dev Enable metamask when the user click the "connect to a wallet" button
   */
